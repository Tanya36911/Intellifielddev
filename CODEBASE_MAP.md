...
| `apps/manager/` | FRONTEND | The Manager web app: the React screens a branch manager (district/region) uses, scoped by the backend to their own branch. Its app shell is live (a scope-forward sidebar with a "Your scope" chip, a locked company card, the branch footprint, and a 6-item menu; a manager/admin-only doorman that bounces field reps); the four real screens (Dashboard, Compliance Review, Survey Assignment, Payroll Approval) are placeholders being built one per step, and Route Planning + Announcements are greyed "soon". It reuses the shared `@intelli/ui`, `@intelli/api-client`, and `@intelli/tokens` packages and keeps its own login. No backend change. Full guide: [apps/manager/README.md](apps/manager/README.md). |
